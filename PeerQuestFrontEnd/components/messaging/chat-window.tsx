...
function ChatWindow({
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-96px)] relative overflow-hidden" style={{ backgroundColor: "var(--tavern-cream)"}}>
      {/* Header */}
      <div className="sticky top-0 z-10 px-4 py-3 border-b" style={{ backgroundColor: "var(--tavern-dark)", borderColor: "var(--tavern-gold)" }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-sm" style={{ color: "var(--tavern-cream)" }}>
                ...
                <span className={isOnline ? "text-green-400 font-medium" : isIdle ? "text-amber-400 font-medium" : "text-slate-300 font-medium"}>
                  {onlineLabel.charAt(0).toUpperCase() + onlineLabel.slice(1)}
                </span>
          ...
          <>
              ...
              return (
                <div key={message.id} className={`flex gap-3 ${isCurrentUser ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className="flex-1 max-w-xs lg:max-w-md xl:max-w-lg">
                    <div className={`px-4 py-2 rounded-2xl shadow-sm ${isCurrentUser ? "ml-auto" : ""}`} style={{
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {message.attachments.map((attachment, aidx) => {
                            ...
                            return isImage && attachment.url
                              ? <MessengerImage key={attachment.url || aidx} url={attachment.url} filename={attachment.filename} />
                              : <MessengerFile key={attachment.url || aidx} url={attachment.url} filename={attachment.filename} />;
                          })}
                        </div>
                      )}
                    </div>
            ...
            {typingUsers.length > 0 && (
              <div className="flex gap-3">
                ...
                <div className="px-4 py-2 rounded-2xl shadow-sm" style={{
                  backgroundColor: "white",
                  border: "1px solid var(--tavern-gold)"
                }}>
                  <div className="flex space-x-1">
                    <div className="w-2 h-2 rounded-full animate-bounce" style={{ backgroundColor: "var(--tavern-gold)" }} />
                    <div className="w-2 h-2 rounded-full animate-bounce" style={{ backgroundColor: "var(--tavern-gold)", animationDelay: "0.1s" }} />
                    <div className="w-2 h-2 rounded-full animate-bounce" style={{ backgroundColor: "var(--tavern-gold)", animationDelay: "0.2s" }} />
                  </div>
                </div>
              </div>
            )}
