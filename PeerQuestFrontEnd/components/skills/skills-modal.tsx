...
export default function SkillsModal({ isOpen, onClose, onSkillsUpdated }: SkillsModalProps) {
  ...
  const loadData = async () => {
    try {
      ...
      if (skillsResponse.success) {
        ...
        const categories = Object.keys(skillsResponse.skills_by_category);
  ...
  const handleSaveSkills = async () => {
    ...
    // Only save skills that exist in available skills
    const availableSkillIds = new Set(
      Object.values(skillsByCategory).flat().map((s: any) => String(s.id))
    );
    const skillsToSave = Array.from(selectedSkills)
      .filter(skillId => !!skillId && uuidRegex.test(skillId) && availableSkillIds.has(skillId))
      .map(skillId => ({
        skill_id: skillId,
                  ...
                  <div className="mb-6 p-4 bg-[#8B75AA]/5 rounded-lg border border-[#8B75AA]/20">
                    ...
                    {userSkills.length > 0 ? (
                      <div className="flex flex-wrap gap-2 mb-2">
                        {userSkills.slice(0, 10).map((skill: any) => (
                          <span
                    ...
                    {selectedSkills.size > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {Array.from(selectedSkills).slice(0, 10).map((skillId) => {
                          const skill = Object.values(skillsByCategory).flat().find((s: any) => s.id === skillId) as any;
                          return skill ? (
                            <span
                              key={skillId}
                  ...
                  {activeCategory && (
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {skillsByCategory[activeCategory]?.map((skill: any) => {
                          // Use string comparison for IDs for robustness
